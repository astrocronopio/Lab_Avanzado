...
def print_to_fit_log(filename,data_energy_item, out, curva):
	...
	data_energy_item.write(" \t %f \t %f \t" %(out.params['L'+str(curva)+'height'].value,out.params['L'+str(curva)+'height'].stderr))
	data_energy_item.write(" \t %f \t %f \t" %(area, err_area))
...
data_energy_list = [open("./L0_09_Mayo_03.txt", "w+"), 
					open("./L1_09_Mayo_03.txt", "w+"), 
					open("./L2_09_Mayo_03.txt", "w+"),
					open("./L3_09_Mayo_03.txt", "w+"), 
					open("./L4_09_Mayo_03.txt", "w+"), 
					open("./L5_09_Mayo_03.txt", "w+")] 
...
L3_mod=VoigtModel(prefix="L3")
L4_mod=VoigtModel(prefix="L4")
L5_mod=VoigtModel(prefix="L5")
...
center=		[1.53183,1.53225,1.53262, 1.53447,1.53528,1.53618] #agrego más semillas de ser necesario
amplitude=	[500,		100,		1000, 800,		1400,		3000	 ]
sigma= 		[0.00001115, 	0.00000115, 	0.00005, 0.00001115, 	0.0000115, 	0.00005]

c=800 # la constante
slope=1000

#Cosas que tengo que hacer con los archivos
header_f= lambda x: data_energy_list[x].write("\n#Centro \t err \t \t \t\t  Fwhm \t err\t\t  Altura \t err \t\t\t\t Area \t Err\t\t\t\t Intensidad \t Err\n")
flush_f = lambda x: data_energy_list[x].flush()
close_f = lambda x: data_energy_list[x].close()

header_f(0), header_f(1), header_f(2)
header_f(3), header_f(4), header_f(5)
...
for filename in files:
	...
	if yes=="y":
		...
		per_peak(3), per_peak(4), per_peak(5)
	...
	fit_log.flush() 
	flush_f(0), flush_f(1), flush_f(2)
	flush_f(3), flush_f(4), flush_f(5) #Agrego mas files de ser necesario

	counter+=1

#Aguante Tapia vieja
fit_log.close()
close_f(0), close_f(1), close_f(2)
close_f(3), close_f(4), close_f(5) #Agrego mas files de ser necesario