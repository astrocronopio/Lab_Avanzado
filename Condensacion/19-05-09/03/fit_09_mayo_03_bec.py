...
data_energy_list = [open("./L0_09_Mayo_03_condesado.txt", "w+"), 
					open("./L1_09_Mayo_03_condesado.txt", "w+")] 
...
center=		[1.53298,1.53563,1.53262, 1.53447,1.53528,1.53618] #agrego más semillas de ser necesario
amplitude=	[50000,		1000,		1000, 800,		1400,		3000	 ]
sigma= 		[0.00001115, 	0.0000115, 	0.00005, 0.00001115, 	0.0000115, 	0.00005]

c=800 # la constante
slope=1000

#Cosas que tengo que hacer con los archivos
header_f= lambda x: data_energy_list[x].write("\n#Centro \t err \t \t \t\t  Fwhm \t err\t\t  Altura \t err \t\t\t\t Area \t Err\t\t\t\t Amplitud\t Err\n")
flush_f = lambda x: data_energy_list[x].flush()
close_f = lambda x: data_energy_list[x].close()

header_f(0), header_f(1)
...
for filename in files:
	...
	if counter==2:
		...
		filtro_total=filtro_3*filtro_2*filtro_1*negro*2.2
	...
	fit_log.flush() 
	flush_f(0), flush_f(1) #Agrego mas files de ser necesario

	counter+=1

#Aguante Tapia vieja
fit_log.close()
close_f(0), close_f(1)#Agrego mas files de ser necesario