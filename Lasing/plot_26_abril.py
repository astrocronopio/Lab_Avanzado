...
mpl.rcParams.update({'font.size': 25,  'figure.figsize': [10, 8],  'figure.autolayout': True})
#######################################################
...
for file in files:
	...
	if counter%3==0:
		plt.plot(1239.8/x, offset_plot+ filtro_total*(y_-a), color=colors[-counter], label=str(y[counter])+" mW")
		plt.text(1.53111 ,75, "L1")
		plt.text(1.53311 ,75, "L2")
	counter+= 1 
...
#cositos
mpl.rcParams.update({'font.size': 24,  'figure.figsize': [15, 8],  'figure.autolayout': True})
...
for x in range(len(files_fit)):
	...
	ax2.set_xticks([5,10,20,30,40,50,60])
